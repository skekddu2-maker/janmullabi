...
country_data = {
    🇰🇷 한국 (Korea) {
        🔍 검색엔진 [
            (네이버 통합검색, lambda q fhttpssearch.naver.comsearch.naverquery={urllib.parse.quote(q)}, #059669),
            (다음 통합검색, lambda q fhttpssearch.daum.netsearchw=tot&q={urllib.parse.quote(q)}, #D97706)
        ],
        🛒 중고 플랫폼 [
            (번개장터, lambda q fhttpsm.bunjang.co.krsearchproductsq={urllib.parse.quote(q)}, #DC2626),
            (당근마켓, lambda q fhttpswww.daangn.comsearch{urllib.parse.quote(q)}, #FF6F0F),
            (후르츠패밀리, lambda q fhttpsfruitsfamily.comsearch{urllib.parse.quote(q)}sort=RELEVANCE, #6366F1)
        ]
    },
    🇯🇵 일본 (Japan) {
        🔍 검색엔진 [
            (야후 재팬 (Yahoo!), lambda q fhttpssearch.yahoo.co.jpsearchp={urllib.parse.quote(q)}, #7C3AED)
        ],
        🛒 중고 플랫폼 [
            (메루카리 (Mercari), lambda q fhttpsjp.mercari.comsearchkeyword={urllib.parse.quote(q)}, #E11D48),
            (야후 옥션 (Yahoo Auction), lambda q fhttpsauctions.yahoo.co.jpsearchsearchp={q.replace(' ', '+')}&s1=cb0, #EA580C)
        ]
    },
    🇺🇸 미국 및 글로벌 (Global) {
        🔍 검색엔진 [
            (구글 (Google), lambda q fhttpswww.google.comsearchq={urllib.parse.quote(q)}, #2563EB)
        ],
        🛒 중고 & 빈티지 플랫폼 [
            (이베이 (eBay), lambda q fhttpswww.ebay.comschi.html_nkw={q.replace(' ', '+')}&_sop=15, #1D4ED8),
            (페이스북 마켓플레이스, lambda q fhttpswww.facebook.commarketplacesearchquery={urllib.parse.quote(q)}, #1E40AF),
            (빈티드 (Vinted), lambda q fhttpswww.vinted.frcatalogsearch_text={urllib.parse.quote(q)}, #0D9488),
            (포시마크 (Poshmark), lambda q fhttpsposhmark.comsearchquery={urllib.parse.quote(q)}, #BE185D),
            (그레일드 (Grailed), lambda q fhttpswww.grailed.comshopquery={urllib.parse.quote(q)}, #4B5563),
            (디팝 (Depop), lambda q fhttpswww.depop.comsearchq={urllib.parse.quote(q)}, #EF4444),
            (베스티에르 콜렉티브, lambda q fhttpswww.vestiairecollective.comsearchq={urllib.parse.quote(q)}, #374151)
        ]
    }
}
...
def _on_mousewheel(event)
    canvas.yview_scroll(int(-1(event.delta120)), units)
